...
export default function DriverLicenseScreen({ navigation }) {
  ...
  const validateLicenseNumber = (text) => {
    // Ejemplo: validar que solo contenga números y letras, y longitud mínima 5
  ...
  const handleSave = () => {
    if (!licenseNumber.trim()) {
      Alert.alert('Error', 'Por favor ingrese el número de licencia.');
      return;
    }
    if (!validateLicenseNumber(licenseNumber)) {
      Alert.alert('Error', 'Número de licencia inválido.');
      return;
    }
    ...
    Alert.alert('Guardado', 'Licencia de conducir guardada correctamente.');
    navigation.goBack();
  };

  const isSaveDisabled = !licenseNumber.trim() || !frontPhoto || !backPhoto;
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.inputGroup}>
        <Text style={styles.label}>Licencia de conducir (frente)</Text>
      ...
      <View style={styles.inputGroup}>
        <Text style={styles.label}>Licencia de conducir (parte trasera)</Text>
